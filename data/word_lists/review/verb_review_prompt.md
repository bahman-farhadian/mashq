...
- `english_verb_review.zip`
- `german_verb_review.zip`

Each archive contains every available CEFR level for that language and its matched vocabulary/sentence review JSON files for **verbs**.
...
- `<language>_vocabulary_<level>_verb_review.json`
- `<language>_sentences_<level>_verb_review.json`
...
One language-specific ZIP archive is attached: either `english_verb_review.zip` or `german_verb_review.zip`.
...
1. `<language>_vocabulary_<level>_verb_review.json`
2. `<language>_sentences_<level>_verb_review.json`
...
Verb-specific rules:
- Every `word` value must be the infinitive/base form of the verb.
- Do not use finite forms, participles, imperatives, nouns, or English infinitives such as `to go` as a verb headword.
- Preserve separable and inseparable prefixes as part of the infinitive.
- Keep irregular and strong verbs in their correct infinitive; never regularize them.
- Keep reflexive information in the definition and example where needed. Do not replace the headword with a conjugated form.
- Ensure each example uses a grammatically correct conjugated form of the verb headword.
- Preserve the verb's meaning and do not introduce conjugation tables, pronoun drills, or a different part of speech.
- For English, use the ordinary base verb form, not a third-person, past, gerund, or participle form unless it is a separate lexical entry.
