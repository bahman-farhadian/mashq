# Adjective Dataset Review

Upload one language-specific ZIP archive to the model:

- `english_adjective_review.zip`
- `german_adjective_review.zip`

Each archive contains every available CEFR level for that language and its matched vocabulary/sentence review JSON files for **adjectives**.
...
- `<language>_vocabulary_<level>_adjective_review.json`
- `<language>_sentences_<level>_adjective_review.json`
...
One language-specific ZIP archive is attached: either `english_adjective_review.zip` or `german_adjective_review.zip`.
...
1. `<language>_vocabulary_<level>_adjective_review.json`
2. `<language>_sentences_<level>_adjective_review.json`
...
- Every `word` value must be the positive dictionary form of the adjective.
- Do not use inflected forms, comparative forms, or superlative forms unless the form is a distinct lexical entry with a different meaning.
- Correct agreement, declension, spelling, capitalization, and example-sentence grammar only where required.
- Preserve the intended meaning and CEFR difficulty.
- Do not reclassify determiners, particles, adverbs, verbs, or nouns into adjectives.
- For German, retain adjective headwords in their uninflected base form; the example may use the grammatically required inflected form.
