...
- `english_noun_review.zip`
- `german_noun_review.zip`

Each archive contains every available CEFR level for that language and its matched vocabulary/sentence review JSON files for **nouns**.
...
- `<language>_vocabulary_<level>_noun_review.json`
- `<language>_sentences_<level>_noun_review.json`
...
One language-specific ZIP archive is attached: either `english_noun_review.zip` or `german_noun_review.zip`.
...
1. `<language>_vocabulary_<level>_noun_review.json`
2. `<language>_sentences_<level>_noun_review.json`
...
Noun-specific rules:
- German countable nouns must use exactly: `der/die/das Singular, die Plural`.
- Correct German definite articles, singular forms, plural forms, capitalization, umlauts, and spelling.
- A German uncountable or singular-only noun must use one correct article+noun form only. Do not invent, repeat, or append a plural.
- Preserve `NICHT ZÄHLBAR` only when the noun is genuinely uncountable or singular-only; it must remain at the end of the relevant vocabulary definition.
- A German plural-only noun must use only its correct plural form with article, for example `die Leute`.
- Remove source artifacts such as fragments and optional endings like `(e)` or `(en)` only when the correct standard form is certain.
- Do not force a plural onto a genuine uncountable, singular-only, or plural-only noun.
- For English nouns, use the normal dictionary headword. Preserve valid plural-only and uncountable forms, and do not apply German article/plural notation.
