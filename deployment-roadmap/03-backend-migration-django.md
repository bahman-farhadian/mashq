# 03 — Backend Migration: `tartarus.py`/`tartarus_web.py` → Django
...
## 3.1 Django apps and what each owns

| App | Owns | Ported from |
|---|---|---|
| `accounts` | Users, auth, sessions (real auth — see §3.5) | `users` table + the missing auth layer |
...
| `analytics` | Fire-and-forget event emission into Mongo | (new — see `05`) |

## 3.2 Model shape (field lists, not code — the actual `models.py` is
...
need it and shouldn't have it (see `01` §3 for why Postgres is the target).
...
## 3.3 The live/in-progress session: Redis, not a table

The legacy app's `SESSIONS` dict (`tartarus_web.py`) — an in-process
dictionary of currently-active sessions, guarded by a `threading.RLock`,
swept periodically by `cleanup_sessions()` against a TTL — is exactly the
shape of state Redis exists for: short-lived, high-churn, doesn't need to
survive a process restart, doesn't belong in the durable relational schema.

Port target: one Redis key per active session (`session:<uuid>`, a JSON
blob mirroring the legacy `session` dict's shape — `queue`, `current`,
`practiced`, `correct`, etc.), with a `SETEX` TTL equal to
`TARTARUS_SESSION_TTL_SECONDS`. Redis's native key expiry replaces the
bespoke `cleanup_sessions()` sweep entirely — one less piece of code to
maintain, and it's correct under multi-process/multi-node deployment in a
way an in-process Python dict never could be (this matters starting at
Stage 2, the moment there's more than one backend process).

## 3.4 The engine stays framework-agnostic
...
in §3.6 possible without spinning up a test client for every case.

## 3.5 Authentication (closing the gap from `00`)
...
- Session-based auth for the Next.js frontend (same-origin, cookie-based —
  ...
  simulator in `11`, which needs to authenticate as up to ~1000 distinct
...
## 3.6 Parity testing: the gate before any deployment work starts
...
- **Migration tests** (mirrors `MigrationContractTest`): the one-time
  legacy-data-import management command (§3.7) gets the same treatment the
...
## 3.7 Cutover plan for existing data
...
3. `manage.py migrate_legacy_data --db-path <path>` — same read, now
   ...
   app's own migration discipline (§3.6).
4. Verification pass: row counts per table, spot-check a handful of users'
   ...
   for the same user.
5. `legacy/` stays in the repo and stays runnable (see `02`) — it is the
   reference implementation this comparison is checked against, not a
   throwaway.

## 3.8 Two pieces that cannot survive unchanged: audio and live TTS
...
- **Pre-generated pronunciation** (`data/audio/*.db`, one SQLite file per
  ...
  media store: local filesystem in Stage 1, S3-compatible object storage
  (MinIO, self-hosted, same S3 API as AWS) from Stage 2 onward, since a
  scalable multi-replica deployment can't rely on a file living on one
  node's local disk. `generate_audio_database.py` becomes a Django
  management command (or a Celery task, triggered from the admin when new
  content is added) that writes into that store instead.
...
Next: [04 — Frontend Migration to Next.js](04-frontend-migration-nextjs.md).
